...
@export var oob_margin: float = 200.0
...
## Shared respawn logic — resets position, HP, velocity, and state.
func _respawn() -> void:
	current_hp = max_hp
...
## Respawn the player if they leave the visible screen area.
## Uses the camera's visible rect when a Camera2D is active,
## otherwise falls back to the viewport size around the origin.
func _check_out_of_bounds() -> void:
	var cam := get_viewport().get_camera_2d()
	var screen_rect: Rect2
	if cam:
		# Camera-centered visible area
		var vp_size := get_viewport_rect().size / cam.zoom
		screen_rect = Rect2(cam.global_position - vp_size * 0.5, vp_size)
	else:
		# No camera — use viewport rect at world origin
		var vp_size := get_viewport_rect().size
		screen_rect = Rect2(Vector2.ZERO, vp_size)

	# Expand the rect by the margin so small overshoots don't trigger respawn
	screen_rect = screen_rect.grow(oob_margin)
